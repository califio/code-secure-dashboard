    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            page.Header().AlignRight().Inlined(line =>
            {
                ...
                line.Item().Text("CODE SECURE").FontColor(Colors.White).FontSize(20).Bold();
            });
            page.Content().AlignMiddle().AlignCenter().Column(col =>
            {
                ...
                col.Item().AlignRight().Text("by Code Secure").FontSize(24).FontColor(Colors.White).Bold();
            });
        });
        container.Page(page =>
            {
        ...
        container.Column(col =>
        {
            ...
            col.Item().Table(table =>
            {
                ...
                table.Cell().Element(CellStyle)
                    .Hyperlink(RepoHelpers.GetCommitUrl(model.SourceType, model.RepoUrl, model.CommitSha))
                    ...
                    colFinding.Item().Table(tb =>
                    {
                        ...
                        if (!string.IsNullOrEmpty(finding.Location))
                        {
                            ...
                            tb.Cell().Border(1).Padding(5)
                                .Hyperlink(RepoHelpers.UrlByCommit(model.SourceType, model.RepoUrl, model.CommitSha, finding.Location, finding.StartLine, finding.EndLine))
                                .Text(location);
